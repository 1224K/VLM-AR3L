...
from agent.critic import DoubleQCritic, MultiCategoricalDoubleQCritic
from agent.actor import DiagGaussianActor, MultiCategoricalActor
...
class SACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg,
        ...
        self.target_entropy = -torch.tensor(action_dim).to(self.device)
    ...
    def act(self, obs, sample=False):
        ...
        dist = self.actor(obs)
        action = []
        if type(dist) == list: # multi categorical action space
            for d in dist:
                action.append(d.sample() if sample else d.probs.argmax(dim=-1))
    ...
    def update_critic(self, obs, action, reward, next_obs, 
        ...
        if type(dist) == list:
            for d in dist:
                next_action.append(d.sample().unsqueeze(-1))
                log_prob.append(d.log_prob(next_action[-1].unsqueeze(-1)))
            next_action = torch.cat(next_action, dim=-1).unsqueeze(-1)
            log_prob = torch.cat(log_prob, dim=-1).sum(-1, keepdim=True)
        else:
    ...
    def update_critic_state_ent(
        ...
        state_entropy = state_entropy.view(state_entropy.size(0), -1)
    ...
    def update_actor_and_alpha(self, obs, logger, step, print_flag=False):
        ...
        if type(dist) == list:
            for d in dist:
                action.append(d.sample().unsqueeze(-1))
                log_prob.append(d.log_prob(action[-1].unsqueeze(-1)))
            action = torch.cat(action, dim=-1).unsqueeze(-1)
            log_prob = torch.cat(log_prob, dim=-1).sum(-1, keepdim=True)
        else:
            action = dist.rsample()
