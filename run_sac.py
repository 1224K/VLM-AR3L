...
class Workspace(object):
        ...
        for episode in range(self.cfg.num_eval_episodes):
            ...
            if save_additional:
                ...
                for i, image in enumerate(images):
                    save_image_path = os.path.join(save_image_dir, 'step{:07}_episode{:02}_{}_{}.png'.format(self.step, episode, i, rewards[i]))
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.cfg.vlm_label or self.reward in ['clip'] or (self.cfg.image_reward and self.reward not in ["gt_task_reward", "sparse_task_reward"]):
                if "metaworld" in self.cfg.env:
                    rgb_image = self.env.render()
                    rgb_image = rgb_image[::-1, :, :]
                    if "drawer" in self.cfg.task or "sweep" in self.cfg.task:
            ...
            if self.reward == 'learn_from_preference' or self.reward == 'learn_from_score':
                if not self.cfg.image_reward:
                    ...
                    image = image.reshape(1, 3, image.shape[1], image.shape[2])
                    self.reward_model.eval()
                    if traj_images.__len__() < self.cfg.k +1:
                        pre_image = traj_images[0].transpose(2, 0, 1).astype(np.float32) / 255.0
                    else:
                        pre_image = traj_images[-self.cfg.k -1].transpose(2, 0, 1).astype(np.float32) / 255.0
                    pre_image = pre_image[:, ::self.resize_factor, ::self.resize_factor]
                    pre_image = pre_image.reshape(1, 3, pre_image.shape[1], pre_image.shape[2])
                    if self.reward_model.reward_mode == "RL-VLM-F":
                        reward_hat = self.reward_model.r_hat(image)
                    elif self.reward_model.reward_mode == "VLM-AR3L":
                        ...
                        absolute_reward = self.reward_model.r_hat(image)
                        reward_hat = self.cfg.absolute_alpha * absolute_reward + (1 - self.cfg.absolute_alpha) * relative_reward
                    self.reward_model.train()
            ...
            if self.cfg.image_reward and self.reward not in ["gt_task_reward", "sparse_task_reward", "gt_RR_reward"]:
